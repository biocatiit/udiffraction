...
mxPV = 'none'; xi = 0.; xf = 0; xstep = 0
mtPV = 'none'; ti = 0.; tf = 0; tstep = 0
myPV = 'none'; yi = 0.; yf = 0; ystep = 0
MAXMOTORS = 40
...
class ShutterStatusPanel(Frame):
 ...
 def shutterscan(self):
  global shstat; shstat = 1
  if (sh_a_pv.get()):
   self.a.config(bg='green')
  else:
   self.a.config(bg='red'); shstat = 0

  if (sh_d0_pv.get()+sh_d2_pv.get()==2 and sh_d1_pv.get()+sh_d3_pv.get()==0):
   self.d.config(bg='green')
  else:
   self.d.config(bg='red'); shstat = 0

  if (sh_no_pv.get()):
   self.no.config(bg='green')
  else:
   self.no.config(bg='red'); shstat = 0

  if (sh_nc_pv.get()):
   self.nc.config(bg='red'); shstat = 0
  else:
...
class MotorEntry(Frame):
 def __init__(self,master,text):
  ...
  self.go=0
  self.mPV = StringVar();
  self.mPVVAL = StringVar(); self.mPVVAL.set("Motor Name")
 ...
 def zap(self, event):
  ...
  if (self.use.get()):
   try: 
    ...
    self.motor = 'Invalid Motor'; self.mPVVAL.set(self.motor)
    self.label.config(fg="pink")
    exit
   else: 
    if (a > MAXMOTORS or a<=0):
     self.label.config(fg="pink")
     self.motor = 'PV out of range'; self.mPVVAL.set(self.motor)
     exit
    else:
     if (self.motorType.get()==0):
   ...
   self.go=1; self.motorPV = None
   self.motor = 'Disabled'; self.mPVVAL.set(self.motor)
   self.label.config(fg="pink")

  mw.update()
 
 def if2cw(self,event):
  i = self.mi.get(); f = self.mf.get()
  self.center.set((f+i)/2.0) ; self.width.set(f-i)

 def cw2if(self,event):
  c = self.center.get(); w = self.width.get()
  self.mi.set(c-(w/2.0)) ; self.mf.set(c+(w/2.0))
...
class MCAPanel(Frame):
 def __init__(self,master):
  ...
  self.intt = DoubleVar()
  self.MCAv = IntVar()
  self.tch = IntVar() ; self.tch.set(15)
  ...
  self.jcb[2].select() ; self.jcb[3].select(); self.jcb[4].select();self.jcb[5].select()
...
class FilenamePanel(Frame):
 def __init__(self, master, text):
  ...
  self.fileidx = StringVar() ; self.fileidx.set('0001')
...
class MainWindow(Frame):
 def __init__(self,master):
  Frame.__init__(self,master)
  self.statustext = StringVar() ; self.statustext.set("Awaiting Scan Parameters")  
  self.progresstext = StringVar() ; self.progresstext.set("Mode: Initial State")  
  self.pausestate = IntVar(); self.pausestate.set(1)
  self.pausetext = StringVar() ; self.pausetext.set("Pause ||")
...
#  self.mb.grid(columnspan=3,sticky=W)
 ...
 def take_dark(self,darktime=1.):
  ...
  a = j_start_pv.put(1)
  if (a == None):
	print 'ERROR: Failed caput' + `j_start_pv.pvname`
	sys.exit(-1)
 ...
 def check_status(self):
  ...
  self.m.my.zap(None); uy = self.m.my.use.get()
  self.m.mt.zap(None); ut = self.m.mt.use.get()
  self.m.mx.zap(None); ux = self.m.mx.use.get()
  ...
  if (ux):
   ...
   MCA_v =0 
   MCA_R   = 'dxp2:'; MCA_D = 'mca'     # The old AIM had MCA_R='aim_adc1'
   nROI = 4; nMCA = 1
   ...
   mca_data_pv = [] ; mca_live_pv = [] ; mca_real_pv = [] ; mca_dead_pv = []
   mca_ROIl_pv = [] ; mca_ROIh_pv = [] ; mca_ROIn_pv = [] ; mca_ROId_pv = []
   for i in xrange(nMCA):
     mca_ROIl_pv.append([]) ; mca_ROIh_pv.append([])
     mca_ROId_pv.append([]) ; mca_ROIn_pv.append([])
...
##The CCD time is specified in the MAR window
  ...
  nJ = len(scaler)
  #File fun
  filen = self.f.filename.get(); fileidx = self.f.fileidx.get()
  if (filen==''):
    self.go = 0
    self.statusbox.config(fg="pink")
  ...
  while (os.path.exists(filen+'_'+fileidx+'.'+HDFext)):
    ...
    self.update()
    intfi += 1
    fileidx = '%4.4d'%(intfi) ; self.f.fileidx.set(fileidx)
  self.go = 1
 ...
 def scan(self, yPV, yi, yf, ystep, \
   ...
   count = 0; begin_time = 0.;  cycle_time = 0.
   yns = 0; tns = 0; xns = 0
   ...
   for y in xrange(yns+1):
     #If used, move motor
     if (uy):
       ypos = yi+(y*ystep) ; ymove = abs(ypos - ylastpos)
     ...
     for t in xrange(tns+1):
       #If used, move motor
       if (ut):
         tpos = ti+(t*tstep) ; tmove = abs(tpos - tlastpos)
       ...
       for x in xrange(xns+1):
         #If used, move motor
         if (ux):
           xpos = xi+(x*xstep) ; xmove = abs(xpos-xlastpos)
         ...
         #Calculate ETA
         count += 1 
         cycle_time_last = cycle_time ; cycle_time = time.time()
         tt = cycle_time - begin_time ; ct = cycle_time - cycle_time_last
         if (count != 1):
           at = tt/float(count-1)
 ...
 def move_motor_debug(self,zPV,zname,zpos,zmove,zq,zv,zvb,zta,ztrap):
   ...
   if (zq < 0):
     calct = zmove/zv;                          ztype = 'Top-hat'
   elif (zmove >= ztrap):
     calct = (zmove + (zv - zvb)*zta)/zv ;      ztype = 'Trapezoidal'
   elif (zmove < ztrap):
     calct = 2*(sqrt(zvb**2+zq*zmove)-zvb)/zq ; ztype = 'Triangular'
   ...
   for tq in xrange(100):
     ...
     time.sleep(calct/10.)
   t2 = time.time()
   print zname+'              Actual time:  '+'%.3f'%(t2-t1)+' s'+' ('+`tq`+' DMOV queries)'
   if (tq == 99): dmswitch += 1
 ...
 def take_data(self, xPV, tPV, yPV, inttime, trigPV, h5f, isContinuous=False,mcaData=None):
  ...
  if (NoXRF==0):
     ...
     if (a == None):
      print 'ERROR: Failed caput' + `mca_erasestart_pv.pvname`
      sys.exit(-1)
  ...
  if (NoXRF==0):
     ...
     if (a == None):
        print 'ERROR: Failed caput' + `mca_stop_pv.pvname`
        sys.exit(-1)
